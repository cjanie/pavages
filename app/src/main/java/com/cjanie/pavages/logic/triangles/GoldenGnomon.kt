...
class GoldenGnomon(
    ...
    fun decompose(model: DecomposableModel): Array<Decomposable> {
        val (rX, rY) = B.complex + (C.complex - B.complex) / goldenRatio
        var R = Point("R", rX, rY)

        when (model) {
            DecomposableModel.ONE_TRIANGLE_TWO_GNOMONS -> {
            ...
            DecomposableModel.ONE_TRIANGLE_ONE_GNOMON -> return arrayOf(
                GoldenTriangle(B, R, A),
                GoldenGnomon(R, C, A),
            )
            DecomposableModel.ONE_TRIANGLE_ONE_GNOMON_SYM -> {
    ...
    override fun decompose() : Array<Decomposable> {
        return decompose(DecomposableModel.ONE_TRIANGLE_TWO_GNOMONS)
    }
